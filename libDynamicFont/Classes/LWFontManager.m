...
@interface LWFontManager () <NSURLSessionDataDelegate>
...
@property(nonatomic, copy) void (^showProgressBlock)(void); //显示进度条表示真实开始下截
@property(nonatomic, copy) void (^updateProgessBlock)(float);  //更新下载进度及进度条
@property(nonatomic, copy) void (^completeBlock)(void); //完成下载
...
- (NSString *)fontDirectoryPath {
    ...
    LWFLog(@"======fontDirectoryPath:%@",_fontDirectoryPath);
    LWFLog(@"======App Bundle Path:%@",[[NSBundle mainBundle] bundlePath]);
...
+ (BOOL)writeData:(NSData *)data toFilePath:(NSString *)filePath {
    ...
    LWFLog(@"=======font writeToFile:%@",(result ? @"YES" : @"NO"));

//            NSError *error = nil;
//            BOOL success = [fontTask.dataToDownload writeToFile:fontPath options:0 error:&error];
//            if (!success) {
//                LWFLog(@"writeToFile failed with error %@", error);
//            }
    return result;
}
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    LWFLog(@"=======progress:%.4f, dataToDownload:%lli, downloadSize:%lli", fontTask.progress, (long long int) fontTask.dataToDownload.length, fontTask.downloadSize);
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    LWFLog(@"--------%d:%s \n", __LINE__, __func__);
    LWFLog(@"=====completed; error: %@", error);

    if(!error){
        LWFontDownloadTask *fontTask = self.taskDict[[@(task.taskIdentifier) stringValue]];
        if(fontTask){
            //写入文件
...
+ (void)registerFont:(NSString *)fontPath {
    ...
    if (!CTFontManagerRegisterGraphicsFont(font, &error)) {
        ...
        LWFLog(@"Failed to load font: %@", errorDescription);
...
//下载苹果提供的字体
+ (void)downloadAppleFontWithFvoidontName:(NSString *)fontName
                        showProgressBlock:(void (^)(void))showProgressBlock
                  updateProgressBlock:(void (^)(float progress))progressBlock
                            completeBlock:(void (^)(void))completeBlock {
    LWFontManager *sef = [LWFontManager shareInstance];
                    ...
                    if (!errorDuringDownload) {
                        ...
                        if(sef.completeBlock){ //修改UI控件的字体样式
                            if([LWFontManager isAvaliableFont:fontName]){
                                ...
                                LWFLog(@"=====font %@ is Unavaliable",fontName);
                            }
                        }
...
+ (void)saveAppleFontPathWithFontName:(NSString *)fontName{
    ...
    LWFLog(@"====Apple Font URL:%@", fontPathURL.path);
...
@implementation UIFont (LWDFSwizzling)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [UIFont lwdf_swizzleClassMethod:@selector(fontWithName: size:) withMethod:@selector(myFontWithName: size:)];
    });
}
...
+ (UIFont *)myFontWithName:(NSString *)fontName size:(CGFloat)fontSize{
    ...
    if(fontSize > 0){
        ...
        if(fontName && isAvaliable){
            ...
            BOOL exsit = [LWFontManager exsitCustomFontFileWithFontName:fontName];
            if (exsit) {  //如果已经下载过了
                NSString *fontPath = [[LWFontManager shareInstance].fontDirectoryPath stringByAppendingPathComponent:fontName];
                [LWFontManager registerFont:fontPath];
                font = [UIFont myFontWithName:fontName size:fontSize];
            }else{
...
@implementation NSObject (LWDFSwizzling)
...
+ (BOOL)lwdf_swizzleClassMethod:(SEL)origSel withMethod:(SEL)altSel {
    return [object_getClass((id)self) lwdf_swizzleMethod:origSel withMethod:altSel];
}
